...
class Profile(models.Model):
    ...
    Physical_test_Fail = models.BooleanField(default=False)
    test_failed_date = models.DateField(null=True, blank=True)
    retake_test= models.DateField(null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.Physical_test_passed and not self.Learner_issue:
                today = timezone.now().date()
                self.Learner_issue = today
                self.learner_expire = today + timezone.timedelta(days=10)
                self.save()
        if self.Physical_test_Fail and not self.test_failed_date:
            today = timezone.now().date()
            self.test_failed_date = today
            self.retake_test = today + timezone.timedelta(days=7)
            self.save()
