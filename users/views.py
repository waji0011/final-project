...
def generate_id_card(request):
    ...
    user_data = userdata.objects.get(profile=profile)
    return render(request, 'users/generate_id_card.html', {'profile': profile, 'user_data': user_data})
...
def video_view(request):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'users/video.html', {
            'profile': profile, 'user_data': user_data,
...
def registration(request):
    ...
    if request.method == 'POST':
        ...
        if error_message:
            ...
            pdf.drawString(user_x, y - 5 * line_height, 'Bank Name: Askari Bank')
            pdf.drawString(user_x, y - 6 * line_height, 'Branch: Comsats Wah')
            pdf.drawString(user_x, y - 7 * line_height, 'Account No: 12345678900')
            pdf.drawString(user_x, y - 8 * line_height, 'Fee: Rs 1000')
            ...
            pdf.drawString(bank_x, y - 5 * line_height, 'Bank Name: Askari Bank')
            pdf.drawString(bank_x, y - 6 * line_height, 'Branch: Comsats Wah')
            pdf.drawString(bank_x, y - 7 * line_height, 'Account No: 12345678900')
            pdf.drawString(bank_x, y - 8 * line_height, 'Fee: Rs 1000')
...
class RegisterView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            Profile.objects.create(user=user, is_verified=False, otp=str(random.randint(100000, 999999)))
...
class VerifyView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if otp:
            user = Profile.objects.filter(otp=otp, is_verified=False).first()
            if user:
